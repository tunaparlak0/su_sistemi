...
export default function Subscription() {
  ...
  const [formData, setFormData] = useState({
    name: '', surname: '', mail: '', telephone: '', idNo: '', taxNo: '', subscriptionId: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // 3. Vergi No Doğrulama (12 hane ve rakam olmalı)
    if (formData.taxNo) {
      const taxRegex = /^\d{12}$/;
    ...
    // 4. Telefon Numarası Doğrulama (Türkiye formatı: 05 ile başlamalı ve 11 hane olmalı)
        ...
        <div className="max-w-xl w-full mx-auto px-4 py-8">
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <input type="email" placeholder="E-posta (ornek@mail.com)" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" onChange={(e) => setFormData({...formData, mail: e.target.value})} required />
              <input placeholder="Telefon (05XXXXXXXXX)" maxLength={11} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" onChange={(e) => setFormData({...formData, telephone: e.target.value})} required />
              
              <div className="flex gap-4">
                <input placeholder="TC Kimlik No (11 hane)" maxLength={11} className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" onChange={(e) => setFormData({...formData, idNo: e.target.value})} />
                <input placeholder="Vergi No (12 hane)" maxLength={12} className="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" onChange={(e) => setFormData({...formData, taxNo: e.target.value})} />
              </div>
